...
echo "$($PSQL "SELECT DISTINCT name FROM teams RIGHT JOIN games ON teams.team_id = games.winner_id OR teams.team_id = games.opponent_id WHERE games.year = 2014 AND games.round = 'Eighth-Final'")"
...
echo "$($PSQL "SELECT DISTINCT year, name FROM teams INNER JOIN games on teams.team_id =  games.winner_id AND games.round = 'Final' ORDER BY year")"
